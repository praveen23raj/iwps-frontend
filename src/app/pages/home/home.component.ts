...
export class HomeComponent implements AfterViewChecked {
  ...
  /* Dynamic Loading / Generation States */
  isLoading = false;
  ...
  onSuggestionClick(promptText: string): void {
    ...
    setTimeout(() => {
      this.addMessage({
        role: 'assistant',
        content: `I am ready to help you with: "${promptText}". Let's get started!`,
        type: 'text'
      });
    }, 1500);
  }
  ...
  private showCopiedState(content: string): void {
  ...
  downloadText(text: string): void {
    ...
    link.href = url;
    link.download = `wx1-response-${Date.now()}.txt`;
  ...
  async copyImage(base64: string): Promise<void> {
    ...
    try {
      ...
      console.warn('Image clipboard is not supported by this browser.');
    } catch (error) {
  ...
  downloadImage(base64: string): void {
    ...
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `wx1-image-${Date.now()}.png`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
  ...
  private showDownloadedState(content: string): void {
    this.downloadedMessage = content;
    window.setTimeout(() => {
      if (this.downloadedMessage === content) {
